...
class EnvioApp:
    def __init__(self, root):
        ...
        self.insumos_selecionados = []  # Lista de insumos selecionados
    ...
    def criar_tela_solicitacao(self):
        ...
        tk.Label(frame_insumos, text="Selecione os Insumos:",
                 font=("Arial", 10), bg="#e6f2ff", fg="#003366").pack(anchor="w", pady=5)

        # Lista de insumos disponíveis
        self.lista_insumos = ttk.Combobox(frame_insumos, font=("Arial", 10), width=40)
        ...
        botao_adicionar_insumo = tk.Button(frame_insumos, text="Adicionar",
        ...
        self.lista_insumos_adicionados = tk.Listbox(frame_insumos, font=("Arial", 10), height=5, width=50)
        self.lista_insumos_adicionados.pack(fill="x", pady=5)

        # Botão para remover insumo selecionado
        botao_remover_insumo = tk.Button(frame_insumos, text="Remover Selecionado",
                                         font=("Arial", 9), bg="#ff6666", fg="white",
                                         command=self.remover_insumo_selecionado)
        botao_remover_insumo.pack(side=tk.LEFT, padx=5)

        # Carregar insumos disponíveis
        self.carregar_insumos_disponiveis()

        # Frame para descrição da solicitação
        ...
        tk.Button(frame_botoes, text="Enviar Solicitação",
                  ...
                  padx=15, pady=5,
                  command=self.enviar_solicitacao).pack(side=tk.RIGHT, padx=5)
        tk.Button(frame_botoes, text="Limpar",
                  font=("Arial", 10), bg="#cccccc", fg="#333333",
                  padx=15, pady=5,
                  command=self.limpar_formulario).pack(side=tk.RIGHT, padx=5)
        ...
        if insumo_selecionado and quantidade:
            try:
                ...
                with DatabaseManager() as db:
                    ...
                    if insumo:
                        self.insumos_selecionados.append((insumo[0], insumo[2], quantidade))
    ...
    def remover_insumo_selecionado(self):
        """Remove o insumo selecionado da lista de insumos adicionados."""
        selecionado = self.lista_insumos_adicionados.curselection()
        if selecionado:
            self.lista_insumos_adicionados.delete(selecionado)
            self.insumos_selecionados.pop(selecionado[0])

    def limpar_formulario(self):
        """Limpa todos os campos do formulário."""
        self.lista_insumos_adicionados.delete(0, tk.END)
        self.texto_descricao.delete("1.0", tk.END)
        self.insumos_selecionados = []
    ...
    def enviar_solicitacao(self):
        ...
        if not self.insumos_selecionados:
            messagebox.showwarning("Nenhum Insumo", "Adicione pelo menos um insumo à solicitação.")
            return
        ...
        try:
            ...
            with self.database_manager as db:
                ...
                for insumo_id, _, quantidade in self.insumos_selecionados:
                    db.adicionar_insumo_a_solicitacao(solicitacao_id, insumo_id, quantidade)
                        ...
                        with open(arquivo, mode="w", newline="", encoding="utf-8") as f:
                            ...
                            for solicitacao in solicitacoes:
                                with self.database_manager as db:
                                    insumos = db.buscar_insumos_por_solicitacao(solicitacao[0])
                                    insumos_str = ", ".join(
                                        [f"{i[1]} ({i[3]})" for i in insumos]) if insumos else "Nenhum"
